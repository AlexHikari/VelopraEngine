...
void EventBus::Unsubscribe(EventType type, EventCallback callback) {
	...
	// This might be a bit tricky as you can't directly compare std::function objects.
	// One approach is to use a wrapper class around std::function that also stores an ID for comparison.
}
